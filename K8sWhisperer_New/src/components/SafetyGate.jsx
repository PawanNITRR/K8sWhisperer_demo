...
const SafetyGate = ({ remediation, onApprove, onReject }) => {
  ...
  return (
    <div className={`mt-6 rounded-2xl border-2 transition-all duration-500 overflow-hidden shadow-2xl
      ${isHighRisk ? 'border-amber-500 bg-amber-950/20' : 'border-blue-500 bg-blue-950/20'}`}>
      
      {/* Header: Risk Indicator */}
      <div className={`p-4 flex items-center justify-between ${isHighRisk ? 'bg-amber-500' : 'bg-blue-500'}`}>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5">
            ...
            <span className={`text-sm font-bold uppercase ${blast === 'high' ? 'text-red-400' : 'text-green-400'}`}>
              {blast}
            </span>
          </div>
          <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5">
